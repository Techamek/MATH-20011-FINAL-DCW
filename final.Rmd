...
david- authors note if the intervals don't overlap then the colors have significance difference like how since E clips into D you cant say its significantly different in mean price but as you can see some of them are very far apart like D and J kiss kiss
...
david- authors note a big z-score like 2> or <-2 shows how many standard errors the difference between the color groups are like in a normal distribution so like D vs I with fucking -24 is giga abnormal and not explainable to chance suggesting a meaningful difference, p-value is simple if its <0.05 reject null >=0.5 cant reject null
also null and alternative hypothesis are above the statistical analysis section idk where else to put it anyway cheers
